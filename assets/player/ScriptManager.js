...
var ScriptManager = Class.create({
  initialize: function (a) {
    ...
    document.observe(
      kSlideDidDownloadEvent,
      this.handleSlideDidDownloadEvent.bind(this)
    );
    document.observe(
      kSlideDidNotDownloadEvent,
      this.handleSlideDidDownloadEvent.bind(this)
    );
  },
    ...
    switch (m.name) {
      ...
      case "apple:scale":
        ...
        if (k.layers[0].animations.length > 0) {
          ...
          if (l == q) {
            for (
              var r = 0, h = k.layers[1].animations[0].animations.length;
              r < h;
              r++
            ) {
              if (
                k.layers[1].animations[0].animations[r].property ===
            ...
            for (
              var r = 0,
                h = k.layers[1].animations[0].animations[0].animations.length;
              r < h;
              r++
            ) {
              if (
                k.layers[1].animations[0].animations[0].animations[r]
                  .property === "transform.translation.z"
              ) {
            ...
            c = {
              animations: [],
              beginTime:
                k.layers[1].animations[0].animations[0].animations[0]
                  .beginTime + 0.03,
              duration:
                k.layers[1].animations[0].animations[0].animations[0].duration,
              fillMode: "forwards",
              from: { scalar: false },
              property: "hidden",
              timingFunction: "linear",
    ...
    switch (r.name) {
      case "apple:scale":
        ...
        if (q.layers[0].animations.length > 0) {
          ...
          if (a == h) {
            for (
              var g = 0, o = q.layers[1].animations[0].animations.length;
              g < o;
              g++
            ) {
              if (
                q.layers[1].animations[0].animations[g].property ===
                "transform.translation.z"
              ) {
            ...
            for (
              var g = 0,
                o = q.layers[1].animations[0].animations[0].animations.length;
              g < o;
              g++
            ) {
              if (
                q.layers[1].animations[0].animations[0].animations[g]
                  .property === "transform.translation.z"
              ) {
            ...
            i = {
              animations: [],
              beginTime:
                q.layers[1].animations[0].animations[0].animations[0]
                  .beginTime + 0.03,
              duration:
                q.layers[1].animations[0].animations[0].animations[0].duration,
              fillMode: "forwards",
              from: { scalar: false },
              property: "hidden",
              timingFunction: "linear",
        ...
        if (q.layers[0].animations.length > 0) {
          ...
          if (a == h) {
            ...
            i = {
              animations: [],
              beginTime:
                q.layers[2].animations[0].animations[0].animations[0]
                  .beginTime +
                q.layers[2].animations[0].animations[0].animations[0].duration /
                  2,
              duration:
                q.layers[2].animations[0].animations[0].animations[0].duration /
                2,
  ...
  applyScaleFactor: function () {
    ...
    for (var Q = 0, q = this.script.events.length; Q < q; Q++) {
      ...
      for (var O = 0, c = T.effects.length; O < c; O++) {
        ...
        var C = {};
        if (
          M.name === "com.apple.iWork.Keynote.BLTMosaicFlip" ||
          M.name === "com.apple.iWork.Keynote.BLTBlinds"
        ) {
          var G = 0,
            g = 0;
          ...
          C.particleCount = { x: G + 1, y: g + 1 };
        }
        this.applyScaleFactorForLayer(
          M.baseLayer,
          n.baseLayer,
          k,
          M.name,
      ...
      for (var O = 0, t = T.hyperlinks.length; O < t; O++) {
        ...
        for (var H in h.events) {
          ...
          for (var N = 0, c = m.effects.length; N < c; N++) {
            ...
            var C = {};
            if (
              R.name === "com.apple.iWork.Keynote.BLTMosaicFlip" ||
              R.name === "com.apple.iWork.Keynote.BLTBlinds"
            ) {
              var G = 0,
                g = 0;
              for (
  ...
  downloadScript: function (b) {
    ...
    if (this.delegate.getKPFJsonStringForShow) {
      ...
      if (this.script == null) {
        debugMessageAlways(
          kDebugScriptMangaer_DownloadScript,
          "An error occured on the server. KPF header json is null."
        );
        return;
      }
