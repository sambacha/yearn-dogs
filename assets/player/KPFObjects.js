...
var KPFEvent = Class.create({
  ...
  addAnimationsToIdenticalLayer: function (c, f) {
    if (c.animations && c.animations.length > 0) {
      for (var b = 0, e = c.animations.length; b < e; b++) {
        f.animations.push(c.animations[b]);
      }
    }
    if (c.layers && c.layers.length > 0) {
      for (var a = 0, d = c.layers.length; a < d; a++) {
    ...
    for (var h = 0; h < a.length; h++) {
      ...
      if (m.type === "actionBuild") {
        ...
        if (f) {
          ...
          for (var e = 0, c = m.effects.length; e < c; e++) {
      ...
      if (m.baseLayer == null) {
        ...
        if (m.name === "renderMovie") {
          ...
          if (m.type === "actionBuild" && !m.isEmphasisBuild) {
            for (var g = 0, q = this.effects.length; g < q; g++) {
              if (this.effects[g].objectID === b) {
                if (
                  this.effects[g].type === "actionBuild" ||
                  this.effects[g].name === "renderMovie"
                ) {
                  k = true;
                } else {
                  k = false;
  ...
  renderMovie: function (c, m) {
    var h;
    for (var j = 0, f = c.layers.length; j < f; j++) {
      var d = m.objectID;
      var b = m.name;
      var p = m.type;
      var g = m.movie;
  ...
  renderEffect: function (E, C, k) {
    var D;
    for (var z = 0, j = E.layers.length; z < j; z++) {
      var x = C.objectID;
      var G = C.name;
      var h = C.type;
      var v = C.movie;
            ...
            if (C.name === "apple:action-scale" && !A) {
              ...
              if (o && t) {
                ...
                this.setStyle(
                  ...
                  e + "px",
                  m + "px",
                  kPositionAbsolutePropertyValue,
                  null,
                  null
                );
  ...
  renderEvent: function (a) {
    ...
    this.cssRenderer = new KPFCssRenderer(
      this.baseLayer,
      this.sceneIndex,
      this.animationSupported,
      this.effects
    );
    this.cssRenderer.draw(a);
  },
  ...
  animateEffect: function (c) {
    ...
    if (f) {
      ...
      var e = {
        ...
        nodeToSwapId: g,
        textureAssets:
          gShowController.textureManager.slideCache[this.slideIndex]
            .textureAssets,
        effect: c,
    ...
    if (c.effects.length > 0) {
      setTimeout(
        this.handleEffectDidComplete.bind(this, c),
        (c.beginTime + c.duration) * 1000
      );
    }
  },
  animateEffectWillBegin: function (e) {
    ...
    setTimeout(this.handleAnimateEffectDidBegin.bind(this, a), 0);
  },
  handleAnimateEffectDidBegin: function (a) {
    a.style.opacity = 0;
  },
  ...
  overallEndTimeInternal: function () {
    for (var a = 0, d = this.event.effects.length; a < d; a++) {
      var c = 0;
  ...
  calculateOverallEndTime: function (e, c) {
    ...
    for (var a = 0, d = c.effects.length; a < d; a++) {
...
var KPFEffect = Class.create({
  ...
  destroyEffect: function (b) {
    for (var a = 0, c = b.effects.length; a < c; a++) {
...
var KPFCssRenderer = Class.create({
  ...
  initRendererFlags: function (a) {
    if (
      a.type === "transition" &&
      a.name != "apple:ca-swing" &&
      a.name != "com.apple.iWork.Keynote.BLTSwoosh"
    ) {
      this.enableCompositingUsingBackface = true;
      this.enablePreserve3DFromParent = true;
      if (
        isChrome &&
        (a.name === "apple:ca-push" ||
          a.name === "com.apple.iWork.Keynote.BLTFadeThruColor")
      ) {
        this.enableCompositingUsingBackface = false;
        this.enablePreserve3DFromParent = false;
      }
  ...
  addAnimationNode: function (e, d, a, c) {
    ...
    setElementProperty(
      h,
      kTransformStylePropertyName,
      kTransformStylePreserve3DPropertyValue
    );
    if (c.anchorPoint.pointX != 0.5 || c.anchorPoint.pointY != 0.5) {
      setElementProperty(
        h,
        kTransformOriginPropertyName,
        c.anchorPoint.pointX * 100 + "% " + c.anchorPoint.pointY * 100 + "%"
      );
    }
    var b = ["opacity", "doubleSided", "anchorPointZ"];
    if (c.transformOriginValue && c.transformOriginZValue) {
      if (c.animationInfo.name === "apple:ca-revolve") {
        b.push("anchorPoint", "position", "zPosition");
      } else {
        if (c.animationInfo.name === "apple:3D-cube") {
          b.push(
            "anchorPoint",
            "position",
            "zPosition",
    ...
    if (c.transformOriginZValue && b.indexOf(f) === -1) {
      if (Prototype.Browser.Gecko) {
        setElementProperty(
          h,
          kTransformOriginPropertyName,
          c.anchorPoint.pointX * 100 +
            "% " +
            c.anchorPoint.pointY * 100 +
            "% " +
            c.transformOriginZValue +
            "px"
        );
      } else {
        setElementProperty(
          h,
          kTransformOriginZPropertyName,
          c.transformOriginZValue + "px"
        );
        if (navigator.userAgent.lastIndexOf("Chrome/") <= 0) {
          setElementProperty(
            h,
            kTransformPropertyName,
            "translateZ(" + c.transformOriginZValue + "px)"
          );
        }
      }
    }
  ...
  setVideoLayer: function (l, p) {
    ...
    kpfMovie = p[o];
    if (!kpfMovie) {
      for (var h = 0, b = l.layers.length; h < b; h++) {
      ...
      if (l.layers.length === 1) {
        ...
        for (var h = 0, b = l.layers.length; h < b; h++) {
          ...
          var n = false;
          for (var f = 0, c = kpfMovie.parentLayer.layers.length; f < c; f++) {
  ...
  addNode: function (r, E, f) {
    ...
    var C =
      ...
      ak +
      "," +
      (ai + f.bounds.offset.pointX) +
      "," +
      (af + f.bounds.offset.pointY) +
      ")";
    var o = f.bounds;
    var s = o.width;
    var U = o.height;
    ...
    if (f.anchorPoint.pointX != 0.5 || f.anchorPoint.pointY != 0.5) {
      setElementProperty(
        ae,
        kTransformOriginPropertyName,
        f.anchorPoint.pointX * 100 + "% " + f.anchorPoint.pointY * 100 + "%"
      );
    }
    ...
    if (f.isTransition && this.enablePreserve3DFromParent) {
      ...
      if (f.sublayerTransform[11] != 0) {
        setElementProperty(
          ae,
          kPerspectivePropertyName,
          gShowController.stageManager.perspective + "px"
        );
        setElementProperty(
          ae,
          kTransformStylePropertyName,
    ...
    if (M) {
          ...
          if (ah === "gif") {
            ...
            if (W == null) {
              var ag = {
                ...
                src: Y,
                showControls:
                  gShowController.isRecording === false ? true : false,
                movieDiv: ae,
                parentLayer: E,
              };
              W = new KPFMovie(ag);
      ...
      if (f.hasContentsAnimation) {
        ...
        var T;
        var e;
        for (var m = 0, ab = an.length; m < ab; m++) {
    ...
    for (var ac = 0, A = f.layers.length; ac < A; ac++) {
  ...
  draw: function (a) {
    ...
    if (browserPrefix === "webkit") {
      if (this.enableCompositingUsingBackface && isChrome) {
        setElementProperty(
          this.domNode,
          kBackfaceVisibilityPropertyName,
          "hidden"
        );
      } else {
        setElementProperty(
    ...
    for (var b = 0, c = this.layer.layers.length; b < c; b++) {
  ...
  overallEndTimeForLayer: function (b, c) {
    ...
    for (var a = 0, d = b.layers.length; a < d; a++) {
      ...
      if (a.kpfLayer) {
        if (a.movie) {
          this.animateEffect(
            a,
            a.kpfLayer,
            a.currentKPFLayer ? a.currentKPFLayer : null,
            a.movie
          );
        } else {
          this.animateEffect(
  ...
  animateEffect: function (q, b, k, e) {
    var m = q.objectID + "-video";
    if (b.cssAnimationRules.length > 0) {
      var g = k ? "layer" + k.layerId : "layer" + b.layerId;
      var d = document.getElementById(g);
      if (d) {
        this.applyCssAnimation(d, b, k, q);
      } else {
        if (q.currentKPFLayer) {
          g = "layer" + q.currentKPFLayer.layerId;
          if (q.currentKPFLayer.layers[0]) {
            g = "layer" + q.currentKPFLayer.layers[0].layerId;
          }
          d = document.getElementById(g);
          if (d) {
            this.applyCssAnimation(d, b, q.currentKPFLayer, q);
          }
        }
      }
    } else {
      if (b.isRenderMovie && b.animationInfo.name === "renderMovie") {
        var j = gShowController.movieCache;
        if (j) {
          var p = j[m];
          if (p) {
      ...
      if (k) {
        var a = document.getElementById("layer" + k.layerId);
        var l = window.getComputedStyle(a, null);
        var n = l.getPropertyValue(kTransformPropertyName);
        var f = parseTransformMatrix(n);
        var o =
          "matrix(" +
          f[0] +
          "," +
          ...
          f[3] +
          "," +
          b.bounds.offset.pointX +
          "," +
          b.bounds.offset.pointY +
          ")";
        if (q.name === "apple:action-motion-path") {
          setElementProperty(a, kTransformPropertyName, o);
        }
      }
    }
    for (var h = 0, c = b.layers.length; h < c; h++) {
      if (gShowController.movieCache && gShowController.movieCache[m]) {
        this.animateEffect(q, b.layers[h], k, e);
      } else {
        this.animateEffect(q, b.layers[h], k ? k.layers[h] : null, e);
      }
    }
  },
  applyCssAnimation: function (y, h, K, Z) {
    var U = h.cssAnimation;
    var B = h.cssAnimationRules;
    var T = h.cssAnimation.overallEndTime;
    if (h.hasMovie) {
      if (h.animationInfo.type === "buildOut") {
        this.stopMovie(document.getElementById(h.movieId));
      }
    }
    if (isChrome && h.animationInfo.name === "apple:doorway") {
      if (
        h.layers.length === 0 &&
        h.contentsRect.x === 0 &&
        h.contentsRect.y === 0 &&
        h.contentsRect.width === 1 &&
        h.contentsRect.height === 1
      ) {
        y.style.visibility = "hidden";
      }
    }
    var g = U.keyAnimations.bounds;
    if (g) {
      var z = document.styleSheets[1].cssRules;
      var ag = z.length;
      var D = y.id + "-bounds";
      if (K) {
        D = "layer" + h.layerId + "-bounds";
      }
      for (var ac = 0; ac < ag; ac++) {
        var v = z[ac];
        if (v.name === D) {
          while (v.cssRules.length > 0) {
            if (isIE) {
              for (var ab = 0, E = v.cssRules.length; ab < E; ab++) {
                v.deleteRule(ab);
              }
            } else {
              v.deleteRule(v.cssRules[0].keyText);
            }
          }
          for (var ab = 0, A = g.keyframes.length; ab < A; ab++) {
            var k = g.keyframes[ab];
            var V, ad, a, e;
            if (k.keyframe == 0) {
              V = k.value.width;
              ad = k.value.height;
              v.appendRule(
                "0% {" +
                  kTransformPropertyName +
                  ": scaleX(1) scaleY(1); " +
                  kAnimationTimingFunctionPropertyName +
                  ": " +
                  k.timingFunction +
                  ";}"
              );
    ...
    if (h.initialState.rotation !== 0 || h.initialState.scale !== 1) {
      var l = document.getElementById(y.id);
      var H = window.getComputedStyle(l, null);
      var M = H.getPropertyValue(kTransformPropertyName);
      var N = parseTransformMatrix(M);
      var o = "matrix(1,0,0,1," + N[4] + "," + N[5] + ")";
      setElementProperty(l, kTransformPropertyName, o);
      var x = document.getElementById(
        y.id + "-" + escapeTextureId("transform.rotation.z")
      );
      if (x) {
        setElementProperty(
          x,
          kTransformPropertyName,
          "rotateZ(" + h.initialState.rotation + "rad)"
        );
      }
      var F = document.getElementById(
        y.id + "-" + escapeTextureId("transform.scale.x")
      );
      if (F) {
        setElementProperty(
          F,
          kTransformPropertyName,
          "scaleX(" + h.initialState.scale + ")"
        );
      }
      var p = document.getElementById(
        y.id + "-" + escapeTextureId("transform.scale.y")
      );
      if (p) {
        setElementProperty(
          p,
          kTransformPropertyName,
          "scaleY(" + h.initialState.scale + ")"
        );
      }
    }
    for (var G in U.keyAnimations) {
      var u = U.keyAnimations[G];
      var C = u.keyActions;
      var J = C[C.length - 1];
      var I = u.keyframes[u.keyframes.length - 1];
      var ai = this.cssPropertyNameForAction(G);
      var aa = this.cssPropertyValueForActionValue(G, I.value);
      var d = u.repeatCount;
      var n = T + "s";
      var D = "";
      var R = "";
      ...
      if (
        K &&
        h.animationInfo.type === "actionBuild" &&
        kActionBuildKeyAnimations[h.animationInfo.name].indexOf(G) === -1
      ) {
        continue;
      }
      if (G === "doubleSided") {
        var m = document.getElementById(y.id + "-" + escapeTextureId(G));
        if (m) {
          setElementProperty(m, ai, aa);
        }
        continue;
      }
      if (
        (h.animationInfo.name === "apple:ca-revolve" ||
          h.animationInfo.name === "apple:3D-cube") &&
        h.transformOriginValue &&
        h.transformOriginZValue &&
        (G === "anchorPointZ" || G === "zPosition")
      ) {
        continue;
      }
      switch (G) {
        case "opacity":
          ...
          if (K) {
            ...
            R = "layer" + h.layerId + "-opacity";
          } else {
            setElementProperty(y, "opacity", 1);
          }
          break;
        case "hidden":
          ai = "opacity";
        ...
        case "bounds":
          ...
          R = D;
          if (K) {
            R = "layer" + h.layerId + "-bounds";
          }
          var V, ad, w, L;
          var q = u.keyframes[0];
          var W = u.keyframes[u.keyframes.length - 1];
          aa =
            "scaleX(" +
            W.value.width / q.value.width +
            ") scaleY(" +
            W.value.height / q.value.height +
            ")";
          break;
        default:
          D = y.id + "-" + escapeTextureId(G);
          R = D;
          if (K) {
            R = "layer" + h.layerId + "-" + escapeTextureId(G);
          }
          break;
      }
      if (G === "contents") {
        if (Z.type === "smartBuild") {
          var f = y.id + "-contents-canvas";
          var r = document.createElement("canvas");
          r.width = h.initialState.width;
          r.height = h.initialState.height;
          r.setAttribute("id", f);
          var Y = gShowController.scriptManager.slideIndexFromSceneIndex(
            this.sceneIndex
          );
          var b = {
            canvasId: f,
            canvas: r,
            textureAssets:
              gShowController.textureManager.slideCache[Y].textureAssets,
            effect: Z,
            overallEndTime: T,
            element: y,
          };
          var O = this.glContentsRenderer[f];
          if (!O) {
            O = new KNWebGLRenderer(b);
            this.glContentsRenderer[f] = O;
          }
          var ah = document.getElementById(y.id);
          ah.appendChild(r);
          var Q = Z.beginTime * 1000;
          setTimeout(this.animateContentsWillBegin.bind(this, b), Q);
        } else {
          var X = y.id + "-contents-from";
          var ae = y.id + "-contents-to";
          var P = X;
          var t = ae;
          if (K) {
            P = "layer" + h.layerId + "-contents-from";
            t = "layer" + h.layerId + "-contents-to";
          }
          var s = document.getElementById(X);
          var af = document.getElementById(ae);
          setElementProperty(s, "opacity", 0);
          setElementProperty(s, kAnimationNamePropertyName, P);
          setElementProperty(s, kAnimationDurationPropertyName, n);
          setElementProperty(af, "opacity", 1);
          setElementProperty(af, kAnimationNamePropertyName, t);
          setElementProperty(af, kAnimationDurationPropertyName, n);
        }
        continue;
      }
      ...
      if (c) {
        if (G === "anchorPoint") {
          var S = { x: I.value.pointX, y: I.value.pointY };
          if (h.magicMoveOffsetValue) {
            S.x = h.magicMoveOffsetValue.pointX;
            S.y = h.magicMoveOffsetValue.pointY;
          }
          ai = kTransformPropertyName;
          aa = "translateX(" + S.x + "px) translateY(" + S.y + "px)";
        } else {
          if (G === "anchorPointZ") {
            if (
              ...
              ai = kTransformPropertyName;
              aa = "translateZ(" + -h.transformOriginZValue + "px)";
            } else {
              ai = kTransformPropertyName;
              aa = "translateZ(" + h.transformOriginZValue + "px)";
            }
        ...
        if (G !== "hidden") {
          setElementProperty(c, ai, aa);
        }
        setElementProperty(
          c,
          kAnimationFillModePropertyName,
          J.fillMode === "removed" ? "none" : J.fillMode
        );
        setElementProperty(c, kAnimationNamePropertyName, R);
        setElementProperty(c, kAnimationDurationPropertyName, n);
        if (
          d > 1 &&
          (h.animationInfo.name === "apple:action-blink" ||
            h.animationInfo.name === "apple:action-pulse")
        ) {
          setElementProperty(c, kAnimationIterationCountPropertyName, d);
        }
      }
    }
  },
  animateContentsWillBegin: function (d) {
    var c = d.canvasId;
    var i = d.effect;
    var e = d.element;
    var g = this.glContentsRenderer[c];
    g.draw(i);
    if (!g.animationStarted) {
      g.animate();
      g.animationStarted = true;
    }
    var b = e.id + "-contents-from";
    var a = e.id + "-contents-to";
    var h = document.getElementById(b);
    var f = document.getElementById(a);
    setElementProperty(h, "opacity", 0);
    setElementProperty(f, "opacity", 0);
  },
  cssPropertyValueForActionValue: function (a, b) {
    switch (a) {
      ...
      case "translationEmphasis":
        return (
          "translateX(" +
          b.translationEmphasis[0] +
          "px) translateY(" +
          b.translationEmphasis[1] +
          "px) translateZ(" +
          b.translationEmphasis[2] +
          ")"
        );
      ...
      case "scaleEmphasis":
        return (
          "scale3d(" +
          ensureScaleFactorNotZero(b.scaleEmphasis[3]) +
          "," +
          ensureScaleFactorNotZero(b.scaleEmphasis[4]) +
          "," +
          ensureScaleFactorNotZero(b.scaleEmphasis[5]) +
          ")"
        );
...
var KPFLayer = Class.create({
  initialize: function (c, d, b) {
    ...
    this.texturedRectangle = c.texturedRectangle;
    ...
    if (
      this.initialState.transform != null &&
      this.initialState.transform != undefined
    ) {
    ...
    var a =
      this.position.pointX -
      this.initialState.width / 2 -
      (this.anchorPoint.pointX - 0.5) * this.initialState.width;
    var e =
      this.position.pointY -
      this.initialState.height / 2 -
      (this.anchorPoint.pointY - 0.5) * this.initialState.height;
    ...
    this.bounds = {
      ...
      offset: { pointX: a, pointY: e },
      canvasOffset: { pointX: a + b.pointX, pointY: e + b.pointY },
    };
    ...
    if (this.objectID && this.isRenderMovie) {
      ...
      if (this.cssAnimation) {
        ...
        if (h && this.isActionBuild && !this.isEmphasisBuild) {
          ...
          for (var v = 0, j = this.layers.length; v < j; v++) {
            if (this.layers[v].animations.length === 0) {
              this.layers[v].divNames = [
                "opacity",
                "position",
                "transform.rotation.z",
                "transform.scale.x",
          ...
          if (
            (h || this.isActionBuild) &&
            this.divNames.indexOf("opacity") === -1
          ) {
            this.divNames.push("opacity");
          }
          if (r) {
          ...
          if (
            (this.initialState.rotation !== 0 || this.isActionBuild) &&
            this.divNames.indexOf("transform.rotation.z") === -1
          ) {
            this.divNames.push("transform.rotation.z");
          }
          if (u) {
          ...
          if (
            this.textureId &&
            (this.hasBoundsAnimation || this.isActionBuild)
          ) {
            this.divNames.push("bounds");
          }
          if (
            this.textureId &&
            (this.hasContentsAnimation || this.isActionBuild)
          ) {
...
var KPFCssAnimation = Class.create({
        ...
        for (var z = 0, e = q.length; z < e; z++) {
          ...
          if (a.path) {
            ...
            for (var x = 0; C < E - 1; x++) {
              ...
              var A = r.points[0];
              var h = c.points[0];
              var u = { pointX: A[0], pointY: A[1] };
              var D = { pointX: h[0], pointY: h[1] };
              switch (r.type) {
                case "MoveToPoint":
                case "AddLine":
                case "AddCurve":
            ...
            if (a.keyTimes) {
              ...
              for (var x = 0; x < t - 1; x++) {
                ...
                y = {
                  ...
                  timingFunction: a.timingFunctions
                    ? a.timingFunctions[x]
                    : "linear",
                  groupTimingFunction: d,
                  groupBeginTime: v,
          ...
          if (F.keyTimes) {
            ...
            for (var x = 0; x < t - 1; x++) {
              ...
              y = {
                ...
                beginTime: v + s * b,
                duration: (m - s) * b,
                timingFunction: F.timingFunctions
                  ? F.timingFunctions[x]
                  : "linear",
              };
              this.addAction(y, F);
  ...
  createKeyActions: function () {
    for (var e = 0, a = this.animations.length; e < a; e++) {
    ...
    for (var h in this.keyAnimations) {
      ...
      if (h == "anchorPoint") {
        ...
        if (w.isMagicMove) {
          ...
          var k = { pointX: u.x, pointY: u.y };
          var d = this.anchorPointOffset(w, { x: o.to.pointX, y: o.to.pointY });
          var v = { pointX: d.x, pointY: d.y };
          if (k.pointX !== v.pointX || k.pointY != v.pointY) {
            n = { pointX: v.pointX - k.pointX, pointY: v.pointY - k.pointY };
            w.magicMoveOffsetValue = n;
          }
        }
        if (s != 0) {
          var t = { pointX: 0, pointY: 0 };
          if (m > 0) {
            ...
            if (h === "position") {
              if (w.isBuildIn || w.isBuildOut) {
                if (
                  ...
                  e = {
                    pointX: this.roundNum(
                      o.from.pointX - w.initialState.position.pointX
                    ),
                    pointY: this.roundNum(
                      o.from.pointY - w.initialState.position.pointY
                    ),
                  };
                }
              } else {
      ...
      for (var p = 0, b = g.length; p < b; p++) {
        ...
        q.keyframes.push({
          keyframe:
            this.roundNum(o.endKeyframe) - (o.endKeyframe == 100 ? 0 : f),
          keyName: h,
          value: o.to,
          timingFunction: a,
        });
  ...
  createTimingFunctionForAction: function (c) {
    ...
    switch (a.toLowerCase()) {
      ...
      case "custom":
        b =
          "cubic-bezier(" +
          d.timingControlPoint1x +
          "," +
          d.timingControlPoint1y +
          "," +
          d.timingControlPoint2x +
          "," +
          d.timingControlPoint2y +
          ")";
        break;
      case "linear":
        b = "linear";
  ...
  createAnimationRuleForKeyframes: function (d) {
    for (var n in this.keyAnimations) {
      ...
      if (!isIE) {
        ...
        for (var c = 0, a = f.keyframes.length; c < a; c++) {
          o = f.keyframes[c];
        ...
        if (browserVersion >= 10) {
          ...
          gShowController.animationManager.styleSheet.insertRule(
            kKeyframesPropertyName + " " + e + " {" + h + "}",
            0
          );
          d.cssAnimationRules.push(e);
        }
  ...
  createAnimationRuleForKeyframe: function (l, m, g) {
    ...
    var b;
    b =
      this.cssPropertyNameForAction(f) +
      ": " +
      d +
      "; " +
      (j < 100 ? kAnimationTimingFunctionPropertyName + ": " + n + ";" : "");
    var i = j + "% {" + h + b + "}";
    return i;
  },
  cssPropertyValueForActionValue: function (b, a, c) {
    switch (a) {
      ...
      case "translationEmphasis":
        return (
          "translateX(" +
          c.translationEmphasis[0] +
          "px) translateY(" +
          c.translationEmphasis[1] +
          "px) translateZ(" +
          c.translationEmphasis[2] +
          ")"
        );
      case "rotationEmphasis":
      ...
      case "scaleEmphasis":
        return (
          "scale3d(" +
          ensureScaleFactorNotZero(c.scaleEmphasis[3]) +
          "," +
          ensureScaleFactorNotZero(c.scaleEmphasis[4]) +
          "," +
          ensureScaleFactorNotZero(c.scaleEmphasis[5]) +
          ")"
        );
      case "transform.scale":
      case "transform.scale.xy":
