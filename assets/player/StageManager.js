...
var StageManager = Class.create({
  ...
  adjustStageToFit: function (b) {
    if (this.showWidth != 0 && this.stageWidth != 0) {
      ...
      var e = 20;
      var c =
        Math.tan(((Math.PI / 180) * e) / 2) *
        15 *
        (this.showWidth > this.showHeight ? this.showHeight : this.showWidth);
      this.perspective = c;
      setElementProperty(
        b,
        kTransformOriginPropertyName,
        kTransformOriginCenterPropertyValue
      );
      setElementProperty(b, kTransformPropertyName, a);
      setElementProperty(
        b,
        kPerspectiveOriginPropertyName,
        kTransformOriginCenterPropertyValue
      );
      setElementProperty(
        b,
      ...
      setElementPosition(
        b,
        ((d - 1) * this.showHeight) / 2,
        ((f - 1) * this.showWidth) / 2,
        this.showWidth,
        this.showHeight
      );
  ...
  debugGetStageStatistics: function () {
    var a = { numTextures: 0, numPixels: 0, numDegraded: 0 };
