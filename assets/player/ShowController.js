...
var ShowController = Class.create({
  initialize: function () {
    ...
    document.observe(
      kScriptDidDownloadEvent,
      this.handleScriptDidDownloadEvent.bind(this)
    );
    document.observe(
      kScriptDidNotDownloadEvent,
      this.handleScriptDidNotDownloadEvent.bind(this)
    );
    document.observe(
      kStageIsReadyEvent,
      this.handleStageIsReadyEvent.bind(this)
    );
    document.observe(
      kStageSizeDidChangeEvent,
      this.handleStageSizeDidChangeEvent.bind(this)
    );
    ...
    Event.observe(
      this.displayManager.body,
      "click",
      this.handleClickEvent.bind(this)
    );
    document.observe(
      kFullscreenChangeEventName,
      this.handleFullscreenChangeEvent.bind(this)
    );
    Event.observe(window, "resize", this.handleWindowResizeEvent.bind(this));
    this.touchController.registerTapEventCallback(
      this.handleTapEvent.bind(this)
    );
    this.changeState(kShowControllerState_Stopped);
    this.movieCache = null;
    this.audioCache = null;
    this.playbackController = new KPFPlaybackController(
      {},
      this.stageManager.stage
    );
    this.navigatorController = new NavigatorController(
      document.getElementById("slideshowNavigator")
    );
    this.slideNumberController = new SlideNumberController(
      document.getElementById("slideNumberControl")
    );
    this.slideNumberDisplay = new SlideNumberDisplay(
      document.getElementById("slideNumberDisplay")
    );
    this.helpPlacard = new HelpPlacardController(
      document.getElementById("helpPlacard")
    );
    ...
    switch (this.state) {
      case kShowControllerState_DownloadingScript:
        ...
        if (c) {
          ...
          var j = getUrlParameter("currentSlide");
          var a;
          if (j) {
            a = parseInt(j);
          } else {
          ...
          h = this.scriptManager.sceneIndexFromSlideIndex(a - 1);
        }
        if (g.recording) {
          if (g.recording.eventTracks[0].type === "navigation") {
            this.narrationManager = new NarrationManager(g.recording);
            h = this.narrationManager.sceneIndexFromNavigationEvent(
              this.narrationManager.navigationEvents[0]
            );
        ...
        if (h > g.lastSceneIndex) {
          break;
        }
        if (d === kShowModeAutoplay) {
          this.jumpToScene(h, true);
        } else {
          var b = g.events[h];
          var e = b.automaticPlay == 1 || b.automaticPlay == true;
          this.jumpToScene(h, e);
        }
        break;
      default:
        debugMessage(
          kDebugShowController_HandleScriptDidDownloadEvent,
  ...
  positionSlideNumberControl: function () {
    var b =
      (this.displayManager.usableDisplayWidth -
        this.slideNumberController.width) /
      2;
    var a =
      this.displayManager.stageAreaTop +
      this.displayManager.stageAreaHeight -
      (this.slideNumberController.height + 16);
    this.slideNumberController.setPosition(b, a);
  },
  positionSlideNumberDisplay: function () {
    var b =
      (this.displayManager.usableDisplayWidth - this.slideNumberDisplay.width) /
      2;
    var a =
      this.displayManager.stageAreaTop +
      this.displayManager.stageAreaHeight -
      (this.slideNumberDisplay.height + 16);
    this.slideNumberDisplay.setPosition(b, a);
  },
  positionHelpPlacard: function () {
    var b =
      (this.displayManager.usableDisplayWidth - this.helpPlacard.width) / 2;
    var a =
      (this.displayManager.usableDisplayHeight - this.helpPlacard.height) / 2;
  ...
  handleStageSizeDidChangeEvent: function (a) {
    this.touchController.setTrackArea(
      a.memo.left,
      a.memo.top,
      a.memo.width,
      a.memo.height
  ...
  handleTapEvent: function (b) {
    var d = { pointX: b.memo.pointX, pointY: b.memo.pointY };
    var c = b.memo.target;
    var a;
    if (c) {
      a = this.slideNumberFromTarget(c);
    }
    if (a) {
      this.navigatorController.select(a);
    } else {
      this.processClickOrTapAtDisplayCoOrds(d);
    }
  },
  slideNumberFromTarget: function (a) {
    if (!a) {
      return null;
    }
    while (a.slideNumber == null && a.nodeName.toLowerCase() !== "body") {
      a = a.parentNode;
    }
    return a.slideNumber;
  },
  processClickOrTapAtDisplayCoOrds: function (c) {
    ...
    if (this.slideNumberController.isShowing) {
      ...
      this.slideNumberTimeout = setTimeout(
        this.hideAndResetSlideNumberController.bind(this),
        0
      );
      return;
    }
  ...
  handleSwipeEvent: function (b) {
    ...
    var c = a.direction;
    if (
      this.displayManager.navigatorIsShowing &&
      (c === "left" || c === "right")
    ) {
      this.navigatorController.thumbnailSidebar.hide(
        this.navigatorController.leftSidebar
      );
      return;
    }
    if (a.swipeStartX && a.swipeStartX < 150) {
      if (a.direction === "right") {
        this.navigatorController.thumbnailSidebar.show(
          this.navigatorController.leftSidebar
        );
      } else {
        if (a.direction === "left") {
        ...
        if (this.accumulatingDigits) {
          ...
          if (this.script.showMode != kShowModeHyperlinksOnly) {
            ...
            debugMessage(
              ...
              "- can't do it, we're in hyperlinks only mode"
            );
          }
          break;
        }
  ...
  preloadTextures: function () {
    ...
    this.textureManager.loadScene(b);
  },
  unloadTextures: function () {
    ...
    var b = i.slideIndexFromSceneIndexLookup[c];
    var f = this.textureManager.slideCache;
    for (var h in f) {
      if (h < b - 1 || h > b + 1) {
        var d = f[h];
        for (var a in d.textureAssets) {
          var g = d.textureAssets[a];
          if (g) {
            var e = g.getContext("2d");
            if (e) {
              e.clearRect(0, 0, g.width, g.height);
            }
            delete d.textureAssets[a];
          }
        }
        delete this.textureManager.slideCache[h].textureAssets;
        delete this.textureManager.slideCache[h].textureRequests;
        delete this.textureManager.slideCache[h].requested;
        if (d.pdf) {
          d.pdf.destroy();
          delete this.textureManager.slideCache[h].pdf;
        }
        delete this.textureManager.slideCache[h];
      }
    }
  },
  doIdleProcessing: function () {
    ...
    this.createHyperlinksForCurrentStateTimeout = setTimeout(
      function () {
  ...
  truncatedSlideIndex: function (a) {
    return this.truncatedIndex(
      a,
      this.script.lastSlideIndex,
      this.script.loopSlideshow
    );
  },
  ...
  advanceToNextBuild: function (b) {
    ...
    if (
      this.script.showMode === kShowModeHyperlinksOnly &&
      b != "currentSceneDidComplete"
    ) {
      return false;
    }
    ...
    switch (this.state) {
      ...
      default:
        debugMessage(
          kDebugShowController_AdvanceToNextBuild,
          "nextSceneIndex: " +
            this.nextSceneIndex +
            " can't advance now, not in an idle state (currently in '" +
            this.state +
            "' state), queue up action to run in next idle time"
        );
        if (this.queuedUserAction == null) {
          a = true;
    ...
    switch (this.state) {
      ...
      default:
        debugMessage(
          ...
          "can't go back now, not in an idle state (currently in '" +
            this.state +
            "' state)"
        );
        if (this.queuedUserAction == null) {
  ...
  advanceAndSkipBuild: function (b) {
    if (!this.script) {
      return false;
    }
    if (this.script.showMode == kShowModeHyperlinksOnly) {
    ...
    switch (this.state) {
      ...
      default:
        ...
        if (this.queuedUserAction == null) {
          this.queuedUserAction = this.advanceAndSkipBuild.bind(this, b);
        }
        break;
    }
  },
  goBackToPreviousSlide: function (c) {
    ...
    switch (this.state) {
      ...
      case kShowControllerState_Playing:
      case kShowControllerState_IdleAtInitialState:
        var e = this.scriptManager.slideIndexFromSceneIndex(b);
        var d = this.scriptManager.sceneIndexFromSlideIndex(e);
        var a;
        if (e === 0) {
          if (b > 0) {
            a = 0;
      ...
      default:
        debugMessage(
          kDebugShowController_GoBackToPreviousSlide,
          "can't go back now, not in an idle state (currently in '" +
            this.state +
            "' state)"
        );
  ...
  jumpToScene: function (d, c) {
    this.lastSlideViewedIndex = this.scriptManager.slideIndexFromSceneIndex(
    ...
    switch (this.state) {
      case kShowControllerState_Starting:
        var b =
          "position:absolute;background-color:transparent; left:0px; top:0px; width:" +
          this.displayManager.usableDisplayWidth +
          "px; height:" +
          this.displayManager.usableDisplayHeight +
          "px;";
        this.starting = true;
      ...
      default:
        debugMessage(
          kDebugShowController_JumpToScene,
          "can't jump now, currently in '" +
            this.state +
            "' state which does not supports jumping..."
        );
        return;
    }
    if (this.textureManager.isScenePreloaded(d) === false) {
      this.changeState(kShowControllerState_WaitingToJump);
      var a = { sceneIndex: d, automaticPlay: c };
      this.waitForSceneToLoadTimeout = setTimeout(
        this.handleSceneDidNotLoad.bind(this, a),
        kMaxSceneDownloadWaitTime
      );
  ...
  jumpToScene_partFour: function (f, e) {
    this.displayScene(f);
    if (this.starting) {
    ...
    if (e) {
      var a = this.script;
      if (a.showMode === kShowModeAutoplay) {
        var d = a.events[f];
        var c = d.effects;
        if (c && c.length > 0) {
          var b =
            c[0].type === "transition"
              ? a.autoplayTransitionDelay
              : a.autoplayBuildDelay;
          setTimeout(
            function () {
              this.playCurrentScene();
            }.bind(this),
  ...
  displayScene: function (g, b) {
    if (g === -1) {
      return;
    }
    this.animationManager.deleteAllAnimations();
    var c = this.scriptManager.slideIndexFromSceneIndex(this.currentSceneIndex);
    var e = b ? b.slideIndex : this.scriptManager.slideIndexFromSceneIndex(g);
    if (c !== e) {
      this.resetMediaCache();
    }
    this.setCurrentSceneIndexTo(g);
    if (b) {
      this.playbackController.renderEvent(b);
    } else {
      var f = this.script.slideIndexFromSceneIndexLookup[g];
      var d = this.script.slideList[f];
      var a = new KPFEvent({
        slideId: d,
        slideIndex: f,
        sceneIndex: g,
  ...
  playCurrentScene: function (b) {
    var h = this.state;
    var j;
    var e = 0;
    var d = this.playbackController.eventOverallEndTime();
    ...
    if (b) {
      j = b.sceneIndexToJump;
      this.resetMediaCache();
    } else {
      j = this.nextSceneIndex;
      var g = this.scriptManager.slideIndexFromSceneIndex(
        this.currentSceneIndex
      );
      var c = this.scriptManager.slideIndexFromSceneIndex(j);
      if (g !== c) {
        this.resetMediaCache();
      }
      if (
        this.playbackController.kpfEvent.event.automaticPlay == true &&
        this.playbackController.kpfEvent.event.effects[0].type === "transition"
      ) {
        e = this.playbackController.kpfEvent.event.effects[0].beginTime;
        d = this.playbackController.kpfEvent.event.effects[0].duration;
      }
    }
    if (this.animationSupported) {
      clearTimeout(this.animateTimeout);
      var a = this.playbackController.kpfEvent.event.effects;
      if (a.length === 0) {
        this.animateTimeout = setTimeout(
          function () {
            setTimeout(
              this.currentSceneDidComplete.bind(this, j),
              d * 1000 + 100
            );
          }.bind(this),
          e * 1000
        ...
        var i;
        if (a[0].type === "transition") {
          if (isIE || isEdge) {
            i = this.playbackController.renderEffects();
          } else {
            if (a[0].name != "com.apple.iWork.Keynote.BLTBlinds") {
              i = this.playbackController.renderEffects();
            }
          }
        }
        this.animateTimeout = setTimeout(
          function (k) {
            if (k == null) {
              k = this.playbackController.renderEffects();
            }
            this.playbackController.animateEffects(k);
            setTimeout(
              this.currentSceneDidComplete.bind(this, j),
              d * 1000 + 100
            );
      ...
      var f = this.script.events[this.currentSceneIndex].automaticPlay;
      if (j === -1) {
        this.updateNavigationButtons();
        if (this.delegate.getKPFJsonStringForShow) {
          if (f) {
        ...
        if (f) {
          setTimeout(
            function () {
              this.changeState(kShowControllerState_IdleAtInitialState);
              this.jumpToScene(j, this.script.events[j].automaticPlay);
            }.bind(this),
            2000
          );
        } else {
          this.changeState(kShowControllerState_IdleAtInitialState);
          setTimeout(
            this.jumpToScene.bind(this, j, this.script.events[j].automaticPlay),
            100
          );
        }
      }
  ...
  currentSceneDidComplete: function (c) {
    var b = this.script;
    var d = b.showMode;
    if (this.slideNumberDisplay.isShowing) {
      this.slideNumberDisplay.hide();
    ...
    this.changeState(kShowControllerState_IdleAtFinalState);
    if (d == kShowModeHyperlinksOnly || (c != -1 && c != this.nextSceneIndex)) {
      var e = b.events[c];
      var a = e.automaticPlay == 1 || e.automaticPlay == true;
      this.jumpToScene(c, a);
    } else {
      if (this.nextSceneIndex === -1) {
        this.updateNavigationButtons();
        if (this.delegate.getKPFJsonStringForShow) {
        ...
        if (
          b.events[this.nextSceneIndex].automaticPlay ||
          d === kShowModeAutoplay
        ) {
          runInNextEventLoop(
            this.advanceToNextBuild.bind(this, "currentSceneDidComplete")
          );
        }
      }
    }
  ...
  resetMovieCache: function () {
    for (var a in this.movieCache) {
      delete this.movieCache[a].videoElement;
  ...
  resetAudioCache: function () {
    for (var a in this.audioCache) {
      this.audioCache[a].pause();
      this.audioCache[a].src = "";
  ...
  playNextItemInSoundTrack: function () {
    ...
    this.soundTrackPlayer.src = "../" + a;
    this.soundTrackPlayer.volume = this.script.soundtrack.volume;
    this.soundTrackPlayer.observe(
      "ended",
      this.soundTrackItemDidComplete.bind(this),
      false
    );
  ...
  findHyperlinkAtCoOrds: function (b) {
    ...
    for (var d = a; d > 0; d--) {
      ...
      if (
        b.pointX >= hyperlinkLeft &&
        b.pointX <= hyperlinkRight &&
        b.pointY >= hyperlinkTop &&
        b.pointY <= hyperlinkBottom
      ) {
        return e;
      }
    }
  ...
  createHyperlinks: function (i) {
    if (i === -1) {
      return;
    }
    var j = this.script.events[i];
    if (j == null) {
      return;
    }
    var e = j.hyperlinks;
    ...
    var b = 150;
    var p = 50;
    var c = this.displayManager.showWidth;
    var h = this.displayManager.showHeight;
    for (g = 0; g < l; g++) {
      var d = e[g];
      var o = d.targetRectangle;
      var m = { targetRectangle: o, events: d.events, url: d.url };
      var n = o.x;
      var f = o.y;
      var q = c - (o.x + o.width);
      var k = h - (o.y + o.top);
      this.stageManager.addHyperlink(m.targetRectangle);
      this.activeHyperlinks[g] = m;
    }
    if (this.movieHyperlinks.length > 0) {
      for (var a = 0; a < this.movieHyperlinks.length; a++) {
        var r = this.movieHyperlinks[a];
        this.stageManager.addHyperlink(r.targetRectangle);
        this.activeHyperlinks[g++] = r;
      }
    }
  },
});
