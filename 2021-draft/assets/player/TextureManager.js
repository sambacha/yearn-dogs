var TextureManager = Class.create({
  ...
  setSceneDidLoadCallbackHandler: function (a, b) {
    this.sceneDidLoadCallbackHandler = { handler: a, sceneIndex: b };
  },
  ...
  processSceneDidLoadCallback: function (a) {
    if (
      ...
      a === this.sceneDidLoadCallbackHandler.sceneIndex &&
      this.isScenePreloaded(a)
    ) {
      this.callSceneDidLoadCallback();
    }
  },
  ...
  requestPdfDocument: function (g) {
    ...
    if (window.location.protocol === "file:") {
      ...
      PDFJS.getDocument(b).then(
        this.handleRequestPdfDocumentCompleted.bind(this, c)
      );
    }
  },
  handleRequestLocalPdfCallback: function (b) {
    ...
    var e = atob(b.pdf);
    var c = { slideId: a, slideIndex: d };
    PDFJS.getDocument({ data: e }).then(
      this.handleRequestPdfDocumentCompleted.bind(this, c)
    );
  },
  ...
  handleRequestPdfPageCompleted: function (a, i, h) {
    ...
    d.height = g.height;
    d.width = g.width;
    var e = { canvasContext: c, background: "rgba(0, 0, 0, 0)", viewport: g };
    var f = this;
    var b = h.render(e);
  ...
  requestSlideSvgmap: function (g) {
    ...
    if (window.location.protocol === "file:") {
      ...
      if (window.svgmap == null || window.svgmap == undefined) {
        window.svgmap = this.handleRequestSlideSvgmapCompleted.bind(
          this,
          null,
          true
        );
  ...
  handleRequestSlideSvgmapCompleted: function (n, l, f) {
    ...
    for (var r in e) {
      ...
      if (i === "svgmap") {
        var k = a[j.index];
        var n = { textureId: r, slideId: d, slideIndex: o };
