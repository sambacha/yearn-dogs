...
var KNWebGLParticleSystem = Class.create({
  ...
  animationWillBeginWithContext: function () {
    ...
    var s = this.uniforms;
    var Y = this.attribs;
    if (s.SpeedMax !== undefined) {
      this._speedMax = this.speedMax();
      q.uniform1f(s.SpeedMax, this._speedMax);
    }
    if (s.RotationMax !== undefined) {
      this._rotationMax = this.rotationMax();
      q.uniform1f(s.RotationMax, this._rotationMax);
    }
    if (s.Duration !== undefined) {
      this._duration = this.duration / 1000;
      q.uniform1f(s.Duration, this._duration);
    }
    var w = this.particleSize;
    var Q = this.objectSize;
    var N = CGRectMake(0, 0, Q.width, Q.height);
    var h = Q.width;
    var n = Q.height;
    var o;
    if (this.pixels) {
      var M = q.createFramebuffer();
      o = new Uint8Array(h * n * 4);
      q.bindFramebuffer(q.FRAMEBUFFER, M);
      ...
      if (q.checkFramebufferStatus(q.FRAMEBUFFER) == q.FRAMEBUFFER_COMPLETE) {
        q.readPixels(0, 0, h, n, q.RGBA, q.UNSIGNED_BYTE, o);
      }
      q.bindFramebuffer(q.FRAMEBUFFER, null);
    }
    var K = Y.TexCoord !== undefined;
    q.useProgram(this.program.shaderProgram);
    var k = (this.attributeBuffers = {});
    k.Position = [];
    k.Center = [];
    k.TexCoord = [];
    k.ParticleTexCoord = [];
    ...
    k.LifeSpan = [];
    k.Color = [];
    var I = this.numberOfVerticesPerParticle;
    for (var P = 0, R = this.particlesHigh; P < R; P++) {
      for (var S = 0, aa = this.particlesWide; S < aa; S++) {
        var l = WebGraphics.makePoint(S, P);
        var G = this.particlesWide * l.y + l.x;
        var O = G * I;
        var X = CGRectMake(S * w.width, P * w.height, w.width, w.height);
        if (K && (S === aa - 1 || P === R - 1)) {
          X = CGRectIntersection(X, N);
        }
        var B = X;
        var A = true;
        if (this.pixels) {
          var ah = h * 4;
          var L = WebGraphics.makePoint4D(0, 0, 0, 0);
          var f = 0;
          var z = B.origin.x + B.size.width;
          var m = B.origin.y + B.size.height;
          for (var p = B.origin.y; p < m; ++p) {
            for (var E = B.origin.x; E < z; ++E) {
              if (E < h && p < n) {
                var G = p * h * 4 + E * 4;
                if (Y.Color !== undefined) {
                  var ae = 1;
                  if (B.size.width > 1) {
                    var D = (2 * (E - B.origin.x)) / B.size.width - 1;
                    var ag = (2 * (p - B.origin.y)) / B.size.height - 1;
                    ae = 1 - D * D + (1 - ag * ag);
                  }
                  var W = o[G];
                  var ac = o[G + 1];
          ...
          A = L.w > 0;
          this.particleSystemVisibilities.push(A);
          if (Y.Color !== undefined) {
            if (f == 0) {
              f = 1;
            }
            var J = 1 / f;
            var T = WebGraphics.multiplyPoint4DByScalar(L, J);
            var v = k.Color;
            KNWebGLUtil.setPoint4DAtIndexForAttribute(T, O, v);
            KNWebGLUtil.setPoint4DAtIndexForAttribute(T, O + 1, v);
            KNWebGLUtil.setPoint4DAtIndexForAttribute(T, O + 2, v);
            KNWebGLUtil.setPoint4DAtIndexForAttribute(T, O + 3, v);
          }
        }
        if (this.willOverrideStartingPoints) {
          X.origin = WebGraphics.setOrigin(
            X.origin,
            this.startingPointAtIndexPoint(l)
          );
        }
        var j = [];
        j[0] = WebGraphics.makePoint(X.origin.x, X.origin.y);
        if (I > 1) {
          j[1] = WebGraphics.makePoint(X.origin.x + X.size.width, X.origin.y);
          j[2] = WebGraphics.makePoint(
            X.origin.x + X.size.width,
            X.origin.y + X.size.height
          );
          j[3] = WebGraphics.makePoint(X.origin.x, X.origin.y + X.size.height);
        }
        for (var Z = 0; Z < I; Z++) {
          KNWebGLUtil.setPoint2DAtIndexForAttribute(j[Z], O + Z, k.Position);
        }
        if (Y.Center !== undefined) {
          var e = X.origin.x + X.size.width / 2;
          var d = X.origin.y + X.size.height / 2;
          KNWebGLUtil.setPoint2DAtIndexForAttribute(
            WebGraphics.makePoint(e, d),
            O,
            k.Center
          );
          KNWebGLUtil.setPoint2DAtIndexForAttribute(
        ...
        if (Y.TexCoord !== undefined) {
          for (var Z = 0; Z < I; Z++) {
            var t = WebGraphics.makePoint(j[Z].x / Q.width, j[Z].y / Q.height);
            KNWebGLUtil.setPoint2DAtIndexForAttribute(t, O + Z, k.TexCoord);
          }
        }
        if (Y.ParticleTexCoord !== undefined) {
          KNWebGLUtil.setPoint2DAtIndexForAttribute(
            WebGraphics.makePoint(0, 0),
            O,
            k.ParticleTexCoord
          );
          KNWebGLUtil.setPoint2DAtIndexForAttribute(
            WebGraphics.makePoint(1, 0),
            O + 1,
            k.ParticleTexCoord
          );
          KNWebGLUtil.setPoint2DAtIndexForAttribute(
            WebGraphics.makePoint(1, 1),
            O + 2,
            k.ParticleTexCoord
          );
          KNWebGLUtil.setPoint2DAtIndexForAttribute(
            WebGraphics.makePoint(0, 1),
            O + 3,
            k.ParticleTexCoord
          );
        }
        if (Y.Speed !== undefined) {
          var V = this.speedAtIndexPoint(l);
          KNWebGLUtil.setPoint3DAtIndexForAttribute(V, O, k.Speed);
          KNWebGLUtil.setPoint3DAtIndexForAttribute(V, O + 1, k.Speed);
          KNWebGLUtil.setPoint3DAtIndexForAttribute(V, O + 2, k.Speed);
          KNWebGLUtil.setPoint3DAtIndexForAttribute(V, O + 3, k.Speed);
        }
        if (Y.Rotation !== undefined) {
          var F = this.rotationAtIndexPoint(l);
          KNWebGLUtil.setPoint3DAtIndexForAttribute(F, O, k.Rotation);
          KNWebGLUtil.setPoint3DAtIndexForAttribute(F, O + 1, k.Rotation);
          KNWebGLUtil.setPoint3DAtIndexForAttribute(F, O + 2, k.Rotation);
          KNWebGLUtil.setPoint3DAtIndexForAttribute(F, O + 3, k.Rotation);
        }
        if (Y.Scale !== undefined) {
          var H = this.scaleAtIndexPoint(l);
          KNWebGLUtil.setFloatAtIndexForAttribute(H, O, k.Scale);
          KNWebGLUtil.setFloatAtIndexForAttribute(H, O + 1, k.Scale);
          KNWebGLUtil.setFloatAtIndexForAttribute(H, O + 2, k.Scale);
          KNWebGLUtil.setFloatAtIndexForAttribute(H, O + 3, k.Scale);
        }
        if (Y.LifeSpan !== undefined) {
          var c = this.lifeSpanAtIndexPoint(l);
          KNWebGLUtil.setPoint2DAtIndexForAttribute(c, O, k.LifeSpan);
          KNWebGLUtil.setPoint2DAtIndexForAttribute(c, O + 1, k.LifeSpan);
          KNWebGLUtil.setPoint2DAtIndexForAttribute(c, O + 2, k.LifeSpan);
          KNWebGLUtil.setPoint2DAtIndexForAttribute(c, O + 3, k.LifeSpan);
        }
        if (Y.Color !== undefined && this.willOverrideColors) {
          var C = this.colorAtIndexPoint(l);
          KNWebGLUtil.setPoint4DAtIndexForAttribute(C, O, k.Color);
          KNWebGLUtil.setPoint4DAtIndexForAttribute(C, O + 1, k.Color);
          KNWebGLUtil.setPoint4DAtIndexForAttribute(C, O + 2, k.Color);
          KNWebGLUtil.setPoint4DAtIndexForAttribute(C, O + 3, k.Color);
        }
      }
    }
    var U = 0;
    var ab = (this.elementArray = []);
    for (var Z = 0; Z < this.particleCount; Z++) {
      ab[U++] = 4 * Z + 0;
      ab[U++] = 4 * Z + 1;
      ab[U++] = 4 * Z + 2;
      ab[U++] = 4 * Z + 0;
    ...
    this.buffer = {};
    KNWebGLUtil.bindAllAvailableAttributesToBuffers(
      q,
      Y,
      k,
      {
    ...
    this.elementArrayBuffer = q.createBuffer();
    q.bindBuffer(q.ELEMENT_ARRAY_BUFFER, this.elementArrayBuffer);
    q.bufferData(q.ELEMENT_ARRAY_BUFFER, new Uint16Array(ab), q.DYNAMIC_DRAW);
    if (this.uniforms.Texture) {
      q.uniform1i(this.uniforms.Texture, 0);
    }
    if (this.uniforms.ParticleTexture) {
      q.uniform1i(this.uniforms.ParticleTexture, 0);
    }
  },
  drawFrame: function (d, c) {
    ...
    e.bindBuffer(e.ELEMENT_ARRAY_BUFFER, this.elementArrayBuffer);
    e.bufferData(
  ...
  setColor: function (a) {
    var b = this.gl;
    b.useProgram(this.program.shaderProgram);
    b.uniform4fv(this.uniforms.Color, a);
  },
  ...
  particleSystemSizeWithRequestedNumber: function (d, b) {
    ...
    if (
      ...
      this.willOverrideStartingPoints ||
      d === 1
    ) {
    ...
    if (c < d) {
      ...
      if (a <= 2) {
        return WebGraphics.makeSize(
          Math.ceil(b.width / 2),
          Math.ceil(b.height / 2)
        );
      }
      if (Math.abs(c - d) < Math.abs(e - d)) {
        return WebGraphics.makeSize(
          Math.ceil(b.width / a),
          Math.ceil(b.height / a)
        );
      } else {
        return WebGraphics.makeSize(
          Math.ceil(b.width / (a + 1)),
          Math.ceil(b.height / (a + 1))
        );
      }
    } else {
      do {
  ...
  point3DRandomDirection: function () {
    var b = WebGraphics.randomBetween(-1, 1);
    var c = WebGraphics.randomBetween(0, 2 * Math.PI);
    var d = Math.sqrt(1 - b * b);
    var a = WebGraphics.makePoint3D(d * Math.cos(c), d * Math.sin(c), b);
    return a;
  },
});
var KNWebGLBuildAnvilSmokeSystem = Class.create(KNWebGLParticleSystem, {
  ...
  startingPointAtIndexPoint: function (b) {
    ...
    return WebGraphics.makePoint(
      (a / this.particleCount) * this.objectSize.width - kParticleSize / 2,
      kParticleSize / 2
    );
  },
  speedAtIndexPoint: function (a) {
    ...
    var b = WebGraphics.makePoint3D(
      this.p_anvilGlobalScale() * 5.6 * c,
      this.p_anvilGlobalScale() * 5 * -d,
      0
    );
    return b;
  },
  ...
  lifeSpanAtIndexPoint: function (a) {
    return WebGraphics.makePoint(
      0,
      (this.duration / 1000) * WebGraphics.randomBetween(0.15, 1)
    );
  },
  p_anvilGlobalScale: function () {
    return (
      WebGraphics.mix(
        1.25,
        0.75,
        this.objectSize.width / this.slideSize.width
      ) *
      (this.objectSize.width / 7)
    );
  },
});
var KNWebGLBuildAnvilSpeckSystem = Class.create(KNWebGLParticleSystem, {
  ...
  lifeSpanAtIndexPoint: function (b) {
    var a = this.indexFromPoint(b);
    var c = WebGraphics.makePoint(
      0,
      Math.min(1, this.duration / 1000) * WebGraphics.randomBetween(0.2, 0.5)
    );
    if (a % 3 != 1) {
      c.y *= 10;
    }
...
var KNWebGLBuildFlameSystem = Class.create(KNWebGLParticleSystem, {
  initialize: function ($super, i, f, d, h, c, g, j) {
    ...
    var b = d.width;
    var k = d.height;
    var a = this.particleSystemSizeWithRequestedNumber(
      g,
      WebGraphics.makeSize(b, k)
    );
    var e = WebGraphics.makeSize(
      Math.ceil(b / a.width),
      Math.ceil(k / a.height)
    );
  ...
  p_setupParticleDataWithTexture: function (p) {
    ...
    for (var h = 0, d = this.particleCount; h < d; h++) {
      ...
      KNWebGLUtil.setPoint2DAtIndexForAttribute(
        WebGraphics.makePoint(n.x, n.y),
        b,
        w
      );
      KNWebGLUtil.setPoint2DAtIndexForAttribute(
        WebGraphics.makePoint(n.x + n.width, n.y),
        b + 1,
        w
      );
      KNWebGLUtil.setPoint2DAtIndexForAttribute(
        WebGraphics.makePoint(n.x + n.width, n.y + n.height),
        b + 2,
        w
      );
      KNWebGLUtil.setPoint2DAtIndexForAttribute(
        WebGraphics.makePoint(n.x, n.y + n.height),
        b + 3,
        w
      );
      for (var B = 0; B < 4; B++) {
  ...
  startingPointAtIndexPoint: function (e) {
    var a = 0;
    var g = 0;
    var f = false;
    for (var d = 0; d < this._bottomRowCount; d++) {
      if (this._bottomRow[d].y !== -1) {
        f = true;
        break;
      }
    }
    if (this._bottomRow && f) {
      do {
        var c = WebGraphics.randomBetween(0, this._bottomRowCount - 1);
        c = Math.round(c);
        a = this._bottomRow[c].x;
        g = this._bottomRow[c].y;
      } while (g === -1);
    }
    var b = { x: a * this.objectSize.width, y: g * this.objectSize.height };
    return b;
  },
  speedAtIndexPoint: function (c) {
    ...
    var d =
      (this._actualSize.height * 0.2 + 0.9 * e) *
      (1 + WebGraphics.randomBetween(0, 0.1));
  ...
  rotationAtIndexPoint: function (b) {
    var a = WebGraphics.makePoint3D(
      WebGraphics.randomBetween(-1, 1),
      0,
      WebGraphics.randomBetween(-1, 1)
    );
    return a;
...
var KNWebGLBuildConfettiSystem = Class.create(KNWebGLParticleSystem, {
  ...
  speedAtIndexPoint: function (l) {
    ...
    var h = k * b;
    var c =
      (this.particleSystemSize.width * this.particleSystemSize.width +
        this.particleSystemSize.height * this.particleSystemSize.height) /
      4;
    var f = WebGraphics.makePoint3D(
      WebGraphics.randomBetween(-1, 1),
      WebGraphics.randomBetween(-1, 1),
      WebGraphics.randomBetween(-1, 1)
    );
    ...
    var d = WebGraphics.makePoint3D(
      l.x - this.particlesWide / 2,
      l.y - this.particlesHigh / 2,
      0
    );
...
var KNWebGLBuildDiffuseSystem = Class.create(KNWebGLParticleSystem, {
  initialize: function ($super, j, g, e, i, d, h, k, c) {
    ...
    var a = this.particleSystemSizeWithRequestedNumber(
      h,
      WebGraphics.makeSize(b, l)
    );
    var f = WebGraphics.makeSize(
      Math.ceil(b / a.width),
      Math.ceil(l / a.height)
    );
...
var KNWebGLBuildFireworksSystem = Class.create(KNWebGLParticleSystem, {
  ...
  setupWithTexture: function (d) {
    var g = this.gl;
    var b = d.width;
    var a = d.height;
    var e = -Math.min(this.objectSize.height, this.slideSize.height / 10);
    var c = WebGraphics.randomBetween(e, this.objectSize.height);
    this._startingPoint = WebGraphics.makePoint(
      this.fireworkStartingPositionX * this.objectSize.width,
      c
    );
    var f = WebGraphics.colorWithHSBA(WebGraphics.randomBetween(0, 1), 1, 1, 1);
    this._startingColorRGB = WebGraphics.makePoint3D(f.red, f.green, f.blue);
    this.animationWillBeginWithContext();
  },
  startingPointAtIndexPoint: function (a) {
    return this._startingPoint;
  },
  colorAtIndexPoint: function (c) {
    var e = this.point3DRandomDirection();
    var d = WebGraphics.multiplyPoint3DByScalar(e, this.colorRandomness);
    var b = WebGraphics.addPoint3DToPoint3D(this._startingColorRGB, d);
    b.x = WebGraphics.clamp(b.x, 0, 1);
    b.y = WebGraphics.clamp(b.y, 0, 1);
    b.z = WebGraphics.clamp(b.z, 0, 1);
    var a = { x: b.x, y: b.y, z: b.z, w: 1 };
    return a;
  },
  speedAtIndexPoint: function (c) {
    var d = this.point3DRandomDirection();
    var b = WebGraphics.randomBetween(0.8, 1);
    var a = WebGraphics.multiplyPoint3DByScalar(d, b);
    return a;
  },
  speedMax: function () {
  ...
  scaleAtIndexPoint: function (a) {
    var b = WebGraphics.randomBetween(
      this.randomParticleSizeMinMax.width,
      this.randomParticleSizeMinMax.height
    );
    return b;
  },
  lifeSpanAtIndexPoint: function (a) {
    var b = WebGraphics.makePoint(
      0,
      WebGraphics.randomBetween(this.lifeSpanMinDuration, 1)
    );
    return b;
  },
});
var KNWebGLBuildShimmerSystem = Class.create(KNWebGLParticleSystem, {
  ...
  speedMax: function () {
    var b = Math.sqrt(this.objectSize.width * this.objectSize.height);
    var a = b * 0.075;
    return a;
  },
  speedAtIndexPoint: function (a) {
    var b = this.point3DRandomDirection();
    b.z = 0;
    return b;
  },
});
var KNWebGLBuildShimmerObjectSystem = Class.create(KNWebGLBuildShimmerSystem, {
  initialize: function ($super, i, f, d, h, c, g, j, k) {
    this.willOverrideStartingPoints = false;
    ...
    $super(i, f, d, h, c, a, e, j, k);
    this.setupWithTexture();
  },
  setupWithTexture: function () {
    this.animationWillBeginWithContext();
  },
  drawGLSLWithPercent: function (g, a, i, c) {
    var h = Math.max(0, g * 1.1 - 0.1);
    var f = 1 - Math.min(1, TSDMixFloats(5 * g, g * g, g));
    var d = h * (c ? 1 : -1) * 2;
    var e = CGAffineTransformMakeRotation(d);
    var b = WebGraphics.makeMat3WithAffineTransform(e);
    this.gl.uniformMatrix3fv(this.uniforms.RotationMatrix, false, b);
    this.drawFrame(h, a * f);
  },
});
var KNWebGLBuildShimmerParticleSystem = Class.create(
  KNWebGLBuildShimmerSystem,
  {
    initialize: function ($super, l, h, g, j, f, a, i, d, k, n) {
      this.willOverrideStartingPoints = true;
      this.willOverrideColors = true;
      this.numberOfVerticesPerParticle = 4;
      this.direction = n;
      this.objectSystem = d;
      this.objectSystemVertexCount = d.particleCount * 4;
      var c = (this.kNumHullPoints = 20);
      var m = 0.48828125;
      var e = 0.02;
      var b = 0.3;
    ...
    p_setupVertexData: function () {
      var f = CGRectMake(0, 0, 1, 1);
      var s = this.numberOfVerticesPerParticle;
      var c = this.particleCount;
      var u = this.particleCount * 5;
      var k = this.attributeBuffers;
      var h = {};
      h.Position = [];
      h.Center = [];
      ...
      for (var p = 0; p < c; ++p) {
        var z = p * 4;
        var j = this.attributeBuffers.Scale[z];
        var e = WebGraphics.makePoint3D(
          this.attributeBuffers.Speed[z * 3],
          this.attributeBuffers.Speed[z * 3 + 1],
          this.attributeBuffers.Speed[z * 3 + 2]
        );
        var d = WebGraphics.makePoint4D(
          this.attributeBuffers.Color[z * 4],
          this.attributeBuffers.Color[z * 4 + 1],
          this.attributeBuffers.Color[z * 4 + 2],
          this.attributeBuffers.Color[z * 4 + 3]
        );
        var t = WebGraphics.makePoint(
          this.attributeBuffers.Position[z * 2],
          this.attributeBuffers.Position[z * 2 + 1]
        );
        var x = WebGraphics.makePoint(
          this.attributeBuffers.Position[z * 2 + 4],
          this.attributeBuffers.Position[z * 2 + 5]
        );
        var g = TSDRectWithPoints(t, x);
        if (this.objectSystem.particleSystemVisibilities[p] === false) {
          g = TSDRectWithPoints(
            WebGraphics.makePoint(0, 0),
            WebGraphics.makePoint(0, 0)
          );
        }
        var y = WebGraphics.makePoint(
          this.attributeBuffers.Center[z * 2],
          this.attributeBuffers.Center[z * 2 + 1]
        );
        var m = WebGraphics.makePoint(
          this.attributeBuffers.LifeSpan[z * 2],
          this.attributeBuffers.LifeSpan[z * 2 + 1]
        );
        var l = this.p_particleHullArray;
        var w = this.kNumHullPoints;
        for (var n = 0; n < w; ++n) {
          var r = p * w + n;
          var A = l[n];
          var a = this.p_hullPoint(A, g);
          KNWebGLUtil.setPoint2DAtIndexForAttribute(a, r, h.Position);
          var q = this.p_hullPoint(A, f);
          KNWebGLUtil.setPoint2DAtIndexForAttribute(q, r, h.ParticleTexCoord);
          KNWebGLUtil.setPoint2DAtIndexForAttribute(y, r, h.Center);
          KNWebGLUtil.setPoint4DAtIndexForAttribute(d, r, h.Color);
          KNWebGLUtil.setPoint3DAtIndexForAttribute(e, r, h.Speed);
          KNWebGLUtil.setFloatAtIndexForAttribute(j, r, h.Scale);
          KNWebGLUtil.setPoint2DAtIndexForAttribute(m, r, h.LifeSpan);
        }
      }
      this.attributeBuffers = h;
      var o = (this.elementArray = []);
      var b = 0;
      for (var p = 0; p < u; p++) {
        o[b++] = 4 * p + 0;
        o[b++] = 4 * p + 1;
    ...
    p_hullPoint: function (c, b) {
      var a = WebGraphics.makePoint(
        b.origin.x + b.size.width * c.x,
        b.origin.y + b.size.height * c.y
      );
      return a;
    },
    startingPointAtIndexPoint: function (b) {
      var g = this.indexFromPoint(b) * 4;
      var j = this.objectSystemVertexCount;
      var c = this.objectSystem.attributeBuffers.Position;
      if (g < j) {
        var k = WebGraphics.makePoint(c[g * 2], c[g * 2 + 1]);
        return k;
      }
      var e = this.objectSize.width / 2;
      var h = this.objectSize.height / 2;
      var i = CGPointMake(e, h);
      var a = WebGraphics.randomBetween(0, 1);
      var d = WebGraphics.doubleBetween(0, 2 * Math.PI);
      var f = CGPointMake(i.x + e * a * Math.cos(d), i.y + h * a * Math.sin(d));
      return f;
    },
    speedAtIndexPoint: function (c) {
      var b = this.indexFromPoint(c) * 4;
      var f = this.objectSystemVertexCount;
      var e = this.objectSystem.attributeBuffers.Speed;
      if (b < f) {
        var a = WebGraphics.makePoint3D(e[b * 3], e[b * 3 + 1], e[b * 3 + 2]);
        return a;
      }
      var d = WebGraphics.makePoint3D(0, 0, 0);
      return d;
    },
    scaleAtIndexPoint: function (c) {
      var d = 1;
      var e = 25;
      var b = WebGraphics.randomBetween(0, 1);
      var a = TSUMix(d, e, b * b);
      return a;
    },
    lifeSpanAtIndexPoint: function (e) {
      var d = this.indexFromPoint(e) * 4;
      var f = this.objectSystemVertexCount;
      if (d < f) {
        return WebGraphics.makePoint(0, 1);
      }
      var c = Math.min(2 / (this.duration / 1000), 0.9);
      var b = TSDMixFloats(
        0.01,
        0.99,
        1 - TSUReverseSquare(WebGraphics.randomBetween(0, 1))
      );
      c *= TSUReverseSquare(b);
      b = Math.min(b, 0.99 - c);
      var a = WebGraphics.makePoint(b, c);
      return a;
    },
    colorAtIndexPoint: function (c) {
      var b = this.indexFromPoint(c) * 4;
      var e = this.objectSystemVertexCount;
      var d = this.objectSystem.attributeBuffers.Color;
      if (b < e) {
      ...
      var a = WebGraphics.makePoint4D(1, 1, 1, 1);
      return a;
    },
    drawGLSLWithPercent: function (f, e, h, b) {
      var c = f * (b ? 1 : -1) * 2;
      var d = CGAffineTransformMakeRotation(c);
      var a = WebGraphics.makeMat3WithAffineTransform(d);
      this.gl.uniformMatrix3fv(
        this.uniforms[kShimmerUniformRotationMatrix],
        false,
        a
      );
      var i = 1 - f;
      var j = Math.pow(i, 15);
      var g = TSUMix(i * i, 25 * f, j);
      this.gl.uniform1f(this.uniforms[kShimmerUniformParticleScalePercent], g);
      this.drawFrame(f, e);
    },
  }
);
var KNWebGLBuildSparkleSystem = Class.create(KNWebGLParticleSystem, {
  initialize: function ($super, h, d, c, f, b, a, e, g, i) {
    this.willOverrideStartingPoints = true;
    this.numberOfVerticesPerParticle = 4;
    this.direction = i;
    $super(h, d, c, f, b, a, e, g);
  ...
  p_globalScale: function () {
    var a = this.objectSize;
    var b = this.slideSize;
    var d = Math.min(a.width, a.height);
    var c = d / Math.min(b.width, b.height);
    d = (d / Math.sqrt(Math.sqrt(c))) * 0.25;
    return d;
  },
  startingPointAtIndexPoint: function (b) {
    var d = 0.1 / (this.duration / 1000);
    var c =
      this.direction == KNDirection.KNDirectionTopToBottom ||
      this.direction == KNDirection.KNDirectionBottomToTop;
    var g =
      this.direction == KNDirection.KNDirectionRightToLeft ||
      this.direction == KNDirection.KNDirectionTopToBottom;
    var f = this.indexFromPoint(b);
    var h = f / this.particleCount;
    h = g ? 1 - h : h;
    var j = h + d * WebGraphics.doubleBetween(-1, 1);
    var i = Math.random();
    i = 2 * i - 1;
    i *= Math.abs(i);
    i = (i + 1) / 2;
    var a = c ? i : j;
    var k = c ? j : i;
    var e = WebGraphics.makePoint(
      a * this.objectSize.width - this.particleSize.width / 2,
      k * this.objectSize.height - this.particleSize.height / 2
    );
    return e;
  },
  speedAtIndexPoint: function (b) {
    var d = this.point3DRandomDirection();
    d.z *= 0.01;
    var c = Math.random();
    var a = WebGraphics.multiplyPoint3DByScalar(d, c);
    return a;
  },
  speedMax: function () {
    var b = Math.min(this.objectSize.width, this.objectSize.height);
    var a = b / Math.min(this.slideSize.width, this.slideSize.height);
    b = (b / Math.pow(a, 0.667)) * 0.25 * 1.5;
    return b;
  },
  scaleAtIndexPoint: function (b) {
    var c = this.cachedGlobalScale;
    var a = c / this.particleSize.width;
    return a;
  },
  lifeSpanAtIndexPoint: function (b) {
    var a = this.indexFromPoint(b);
    var d = a / this.particleCount;
    var e = KNSparkleMaxParticleLife / Math.max(0.75, this.duration / 1000);
    d *= 1 - e;
    var c = WebGraphics.makePoint(d, e);
    return c;
  },
});
